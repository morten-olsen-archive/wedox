...
class Client extends EventEmitter<Events> {
  ...
  constructor(host: DataConnection, state: any) {
    super();
    this.#host = host;
  ...
  public send = (action: any) => new Promise((resolve, reject) => {
  ...
  public static create = (hostId: string, config: any) => new Promise<Client>((resolve, reject) => {
    const peer = new Peer();
    peer.on('open', () => {
      try {
        ...
        const callback = (data: any) => {
          if (data.type === 'setup') {
            ...
            resolve(new Client(host, data.payload.state))
          }
        }
